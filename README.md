# Hi there, I'm Krishna 

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=00C0FF&width=900&lines=Data+Scientist+%7C+MLOps+Engineer+%7C+LLM+Enthusiast;Aviation+Geek+%7C+Drone+Hobbyist;Foodie+Exploring+New+Recipes;AI+Copilots%2C+RAG+%26+LLMs;Carnatic+Flute+Player+%26+Badminton+Lover)](https://git.io/typing-svg)

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=sriksven&color=00C0FF&style=flat-square&label=Profile+Views" />
</p>

---

### About Me
- MS in **Data Science** @ Northeastern University (2023–2025)
- Passionate about **LLMs, RAG, MLOps pipelines, and Generative AI**
- Experienced in **deploying AI assistants, financial ML models, and computer vision systems**
- **Aviation enthusiast** fascinated by **ATC comms, flight data, and aviation AI**
- **Drone Hobbyist** exploring aerial photography & drone technology
- Check out my **photography portfolio** [ClickCraft](https://clickcraft.myportfolio.com/)
- Foodie always exploring new **recipes & cuisines**
- Fun fact: I play the **Carnatic Flute**
- Love playing **badminton & squash** and CSGO
- Fitness through **cycling rides** in Boston

---

### Current Focus
- **StratX: Real-Time F1 Telemetry and Machine Learning Race Strategy Engine**
  - End-to-end, fully open-source, real-time Formula 1 race strategy platform
  - Transforms live telemetry into actionable race insights using streaming pipelines, feature stores, machine learning models, and an interactive race strategy dashboard
  - Built with FastF1, OpenF1, Ergast API, Kafka, Flink/Faust, Feast, MLflow, Seldon, Prometheus, and Grafana

- **Real-Time Transit Tracking Application for MBTA**
  - Live tracking for Massachusetts Bay Transportation Authority subway trains, buses, and commuter rail
  - Features live vehicle positions, arrival predictions, and smart commute assistance

---

### Featured Projects
- **StratX: F1 Race Strategy Engine**
  - Real-time telemetry streaming and ML-powered race strategy optimization
  - Tech stack: FastF1, Kafka, Flink/Faust, Feast, MLflow, Seldon, Prometheus, Grafana

- **MBTA Transit Tracker**
  - Real-time transit tracking with live vehicle positions and arrival predictions
  - Smart commute assistance for Boston's public transportation system

- **RAG Literature Assistant (Saint-Gobain)**
  - Serves 100+ researchers daily with 96% faster literature lookups
  - AI-powered research assistant for technical documentation

- **FinAdvisor**
  - LLM-powered financial advisory tool using LoRA/QLoRA fine-tuning
  - Vector storage with Pinecone/Qdrant for efficient retrieval

- **Agri Data Hackathon (USDA) - 1st Place**
  - Winning solution using SARIMA and spatiotemporal modeling
  - Agricultural yield prediction and optimization

- **Real-Time Drowsiness Detection**
  - CNN + OpenCV implementation with 95.6% accuracy
  - Safety system for driver alertness monitoring

---

### Tech Stack & Tools

<p align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/TensorFlow-FF6F00?logo=tensorflow&logoColor=white" />
  <img src="https://img.shields.io/badge/Scikit--learn-F7931E?logo=scikitlearn&logoColor=white" />
  <img src="https://img.shields.io/badge/HuggingFace-F0C929?logo=huggingface&logoColor=black" />
  <img src="https://img.shields.io/badge/LangChain-00A67E?logo=chainlink&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white" />
  <img src="https://img.shields.io/badge/Airflow-017CEE?logo=apacheairflow&logoColor=white" />
  <img src="https://img.shields.io/badge/GoogleCloud-4285F4?logo=googlecloud&logoColor=white" />
  <img src="https://img.shields.io/badge/Azure-0078D4?logo=microsoftazure&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS-FF9900?logo=amazonaws&logoColor=black" />
  <img src="https://img.shields.io/badge/Kafka-231F20?logo=apachekafka&logoColor=white" />
  <img src="https://img.shields.io/badge/Flink-E6526F?logo=apacheflink&logoColor=white" />
  <img src="https://img.shields.io/badge/MLflow-0194E2?logo=mlflow&logoColor=white" />
  <img src="https://img.shields.io/badge/Prometheus-E6522C?logo=prometheus&logoColor=white" />
  <img src="https://img.shields.io/badge/Grafana-F46800?logo=grafana&logoColor=white" />
</p>

---

### Connect with Me

<p align="center">
  <a href="https://sriksven.github.io/personal-website/">
    <img src="https://img.shields.io/badge/Portfolio-orange?style=for-the-badge&logo=google-chrome&logoColor=white" />
  </a>
  <a href="https://clickcraft.myportfolio.com/">
    <img src="https://img.shields.io/badge/Photography-ff69b4?style=for-the-badge&logo=adobecreativecloud&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/sriksven/">
    <img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/sriksven">
    <img src="https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.youtube.com/c/ProfSpud">
    <img src="https://img.shields.io/badge/YouTube-red?style=for-the-badge&logo=youtube&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <img src="https://media.giphy.com/media/26xBukhkP5vUenP4c/giphy.gif" width="60">
  <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="60">
  <img src="https://media.giphy.com/media/j2pOGeGYKe2xCCKwfi/giphy.gif" width="60">
</p>

---

### GitHub Stats
<div align="center">

  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sriksven&layout=compact&theme=radical" />

  <img src="https://github-readme-stats.vercel.app/api?username=sriksven&show_icons=true&locale=en&theme=radical" />

</div>

---

[![Youtube Views](https://img.shields.io/youtube/channel/views/UCBkZGtQ6tQWzCseujiS4CXQ?style=social)](https://www.youtube.com/c/ProfSpud)

*Data is my language, AI is my playground, food is my fuel, and the skies are my inspiration*
